        ...
        <div class="myForm">
          <el-form :inline="true" :model="formInline" class="demo-form-inline">
            <el-form-item label="快捷查询">
              <el-input placeholder="输入快递单号/收件人/手机号" v-model="formInline.name"></el-input>
            </el-form-item>
    ...
    <el-dialog title="订单信息" :visible.sync="dialogVisible" width="800px" :before-close="handleClose">
      <div class="xiangqinBox">
        ...
        <div class="tit1">
          ...
          <div class="txt1">总重量：{{xqObj.weight}}</div>
          <div class="txt1">总价格：{{xqObj.order_amount}}</div>
        </div>
        <div class="tit2">
          <div class="txt1">第三方订单号：{{xqObj.dsf_order}}</div>
          <div class="txt1">快递单号：{{xqObj.kd_order}}</div>
          <div class="txt1">收件人：{{xqObj.sj_username}}</div>
          <div class="txt1">收件人电话：{{xqObj.sj_phone}}</div>
          <div class="txt1">地址：{{xqObj.sj_address}}</div>
        </div>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 查看订单
    async seeDingdan(row) {
      console.log(row);
      const res = await this.$api.orderBg_info({
        token: sessionStorage.getItem("token"),
